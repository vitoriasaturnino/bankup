defmodule Bankup.RecurringAccountsTest do
  ...
  describe "recurring_accounts" do
    ...
    test "create_recurring_account/1 with valid data creates a recurring_account" do
      valid_attrs = %{status: "some status", description: "some description", amount: "120.5", due_date: ~D[2024-10-04], payee: "some payee", pix_key: "some pix_key"}
      ...
      assert recurring_account.amount == Decimal.new("120.5")
      assert recurring_account.due_date == ~D[2024-10-04]
    ...
    test "update_recurring_account/2 with valid data updates the recurring_account" do
      recurring_account = recurring_account_fixture()
      update_attrs = %{status: "some updated status", description: "some updated description", amount: "456.7", due_date: ~D[2024-10-05], payee: "some updated payee", pix_key: "some updated pix_key"}
      ...
      assert recurring_account.amount == Decimal.new("456.7")
      assert recurring_account.due_date == ~D[2024-10-05]
